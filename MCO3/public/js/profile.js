document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const reservationsResponse = await fetch('/api/current-reservations');
    ...
    currentReservations.forEach(reservation => {
      ...
      li.textContent = `Lab ${reservation.labname} - Seat ${reservation.seatnum}, Date: ${new Date(reservation.date).toLocaleDateString()}, Time: ${reservation.time}`;
